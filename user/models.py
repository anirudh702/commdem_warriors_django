...
class UserModel(models.Model):
    ...
    mobile_number = models.CharField(
        ...
        max_length=15,
        db_index=True,
        error_messages={"unique": "This mobile number already exists in our database"},
    )
    email = models.EmailField(
        ...
        unique=True,
        db_index=True,
        error_messages={"unique": "This Email already exists in our database"},
    )
...
class UserPaymentDetailsModel(models.Model):
    ...
    subscription = models.ForeignKey(
        SubscriptionModel, on_delete=models.CASCADE, null=True
    )
...
class UserWisePrivacyModel(models.Model):
    ...
    id = models.AutoField(primary_key=True)
    my_details = models.ForeignKey(
        UserModel,
        on_delete=models.CASCADE,
        null=True,
        related_name="base_user_details",
    )
    other_user_details = models.ForeignKey(
...
class UserFreeTrialPeriodModel(models.Model):
    """Model for storing user free trial period"""
...
class UserAnswerBeforeRelationshipModel(models.Model):
    """Model for storing answers given by users before commitment"""

    id = models.AutoField(primary_key=True)
    user = models.ForeignKey(UserModel, on_delete=models.CASCADE, null=True)
    question = models.ForeignKey(
        QuestionsToAskBeforeModel, on_delete=models.CASCADE, null=True
    )
    answer = models.ForeignKey(mcqsChoicesModel, on_delete=models.CASCADE, null=True)
    created_at = models.DateTimeField(default=django.utils.timezone.now, blank=True)
    updated_at = models.DateTimeField(default=django.utils.timezone.now, blank=True)
    objects = models.Manager()
...
class PaymentForReferralUsersModel(models.Model):
    ...
    to_user = models.ForeignKey(
        UserModel, on_delete=models.CASCADE, null=True, related_name="toUser"
    )
    from_user = models.ForeignKey(
        UserModel, on_delete=models.CASCADE, null=True, related_name="fromUser"
    )
    unique_pay_id = models.BigIntegerField()
    amount = models.FloatField(blank=True, default=0.0)
    referral_payment_status = models.ForeignKey(
        ReferralPaymentStatusModel, on_delete=models.CASCADE, null=True
    )
    created_at = models.DateTimeField(default=django.utils.timezone.now, blank=True)
    updated_at = models.DateTimeField(default=django.utils.timezone.now, blank=True)
    objects = models.Manager()
