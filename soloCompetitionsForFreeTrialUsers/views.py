...
from group_challenges.serializers import GetGroupChallengesSerializer
from response import Response as ResponseData
from soloCompetitionsForFreeTrialUsers.models import (
    FreeTrialSoloChallengesModel,
    RulesOfSoloChallengesModel,
)
from soloCompetitionsForFreeTrialUsers.serializers import (
    UploadVideoOfUserSoloChallengeSerializer,
)
        ...
        if serializer.is_valid():
            ...
            for i in range(0, len(challenges_data)):
                ...
                (
                    ...
                    .last()["date_of_payment"]
                    .date()
                )
                challenges_data[i]["challenge_rules"] = (
                    RulesOfSoloChallengesModel.objects.values().filter().all()
                )
...
def upload_video_for_solo_challenge(request):
    ...
    try:
        ...
        if serializer.is_valid():
            ...
            solo_challenge = FreeTrialSoloChallengesModel.objects.filter(
                id=solo_challenge_id
            ).first()
            ...
            user_challenge_data = ParticipantsInSoloChallengeModel.objects.filter(
                user_id=user_id,
                solo_challenge_id=solo_challenge_id,
                hide_from_user=False,
            ).first()
            if is_updated_file is True:
                print(
                    f"user_challenge_data.challenge_video {user_challenge_data.challenge_video}"
                )
