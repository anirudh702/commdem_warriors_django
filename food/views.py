...
def addFoodType(request):
    ...
    try:
        ...
        if serializer.is_valid():
            ...
            type_of_food_name_exists = TypeOfFoodModel.objects.filter(
                type_of_food_name=type_of_food_name
            ).first()
            if type_of_food_name_exists:
                return Response(
                    ResponseData.error("This food type already exists"),
            ...
            new_data.save()
            return Response(
                ResponseData.success_without_data("Food type added successfully"),
                status=status.HTTP_201_CREATED,
            )
...
def addFoodItem(request):
    ...
    try:
        ...
        if serializer.is_valid():
            ...
            food_image = (
                request.FILES["food_image"] if "food_image" in request.FILES else ""
            )
            ...
            if not type_of_food_name_exists:
                return Response(
                    ResponseData.error("Id of food type is invalid"),
                    status=status.HTTP_201_CREATED,
                )
            food_name_exists = FoodModel.objects.filter(
                Q(food_name__icontains=food_name)
            ).first()
            if food_name_exists:
                return Response(
                    ResponseData.error("This food dish already exists in our database"),
                    status=status.HTTP_201_CREATED,
                )
            if food_image != "":
            ...
            new_data.save()
            return Response(
                ResponseData.success_without_data("Food dish added successfully"),
                status=status.HTTP_201_CREATED,
            )
        for error in serializer.errors:
            print(serializer.errors[error][0])
...
def getAllFoodDishes(request):
    ...
    try:
        ...
        for i in range(0, len(food_dishes_data)):
            food_dishes_data[i]["food_dish_type"] = (
                TypeOfFoodModel.objects.values()
                .filter(id=food_dishes_data[i]["type_of_food_id"])
                .get()
            )
        ...
        if len(food_dishes_data) == 0:
            return Response(
                ResponseData.success([], "No food dish found"),
                status=status.HTTP_201_CREATED,
            )
        return Response(
