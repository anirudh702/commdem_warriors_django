...
from commitment.models import CauseOfCategorySuccessOrFailureModel, CommitmentCategoryModel, CommitmentModel, CommitmentNameModel, ReasonBehindCommitmentSuccessOrFailureForUser
...
class voiceAssistantAfterUpdateMessageModel(models.Model):
    id = models.AutoField(primary_key=True)
    commitment_category = models.ForeignKey(CommitmentCategoryModel, on_delete=models.CASCADE,null=True)
    reason_behind_commitment_success_or_failure = models.ForeignKey(CauseOfCategorySuccessOrFailureModel, on_delete=models.CASCADE,null=True)
...
class voiceAssistantBeforeUpdateMessageModel(models.Model):
    id = models.AutoField(primary_key=True)
    commitment_category = models.ForeignKey(CommitmentCategoryModel, on_delete=models.CASCADE,null=True)
    occupation = models.ForeignKey(DesignationModel, on_delete=models.CASCADE,null=True)
    age_group = models.CharField(max_length=100,blank=True)
    no_of_times_in_current_week = models.IntegerField(blank=True,default=0)
    range_of_success_of_commitment_name_in_this_week = models.CharField(max_length=100,blank=True)
