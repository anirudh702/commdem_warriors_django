...
from oneToOneChatModule.serializers import (
    AddNewChatSerializer,
    GetUsersWithChatSerializer,
)
...
def add_new_chat_between_two_users(request):
    ...
    try:
        ...
        if serializer.is_valid():
            ...
            chat_message = (
                serializer.data["chat_message"]
                if "chat_message" in request.data
                else ""
            )
            files_path = (
                request.FILES["files_path"] if "files_path" in request.FILES else None
            )
            ...
            if files_path is not None:
                for i in range(0, len(files_path)):
                    new_file_data = OneToOneFilesSharedOnChatModel.objects.create(
                        path=files_path[i], chat_id=new_data.id
                    )
                    new_file_data.save()
            ...
            return Response(
                ResponseData.success_without_data("New chat added successfully"),
    ...
    try:
        ...
        if serializer.is_valid():
            ...
            for i in range(0, len(users_data)):
                ...
                mapData = {}
                chat_data = (
                    OneToOneChatModel.objects.filter(
                        (Q(from_user_id=user_id) & Q(to_user_id=users_data[i].id))
                        | (Q(from_user_id=users_data[i].id) & Q(to_user_id=user_id))
                    )
                    .order_by("-created_at")
                    .first()
                )
                ...
                if chat_data is not None:
                    ...
                    mapData["created_at"] = chat_data.created_at + timedelta(
                        hours=5, minutes=30
                    )
            ...
            if len(final_data) == 0:
                return Response(
                    ResponseData.success_without_data("No details found"),
                    status=status.HTTP_201_CREATED,
                )
            return Response(
                ResponseData.success(final_data, "Details fetched successfully"),
                status=status.HTTP_201_CREATED,
            )
        for error in serializer.errors:
            print(serializer.errors[error][0])
