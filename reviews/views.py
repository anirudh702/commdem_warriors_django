...
def get_all_review(request):
    ...
    try:
        ...
        if serializer.is_valid():
            ...
            review_details = list(ReviewModel.objects.values().filter())
            can_user_submit_review = False
            current_date = datetime.now()
            get_user_latest_payment_date = (
                UserPaymentDetailsModel.objects.filter(user_id=user_id, is_active=True)
                .last()
                .date_of_payment
            )
            difference_between_both_dates = (
                current_date - get_user_latest_payment_date.replace(tzinfo=None)
            )
            print(f"difference_between_both_dates {difference_between_both_dates.days}")
            if (
                difference_between_both_dates.days % 15 == 0
                and difference_between_both_dates.days > 14
            ):
                has_user_already_submitted_data_for_today = (
                    UserReviewModel.objects.filter(
                        user_id=user_id, review_date=str(datetime.now()).split(" ")[0]
                    ).first()
                )
                ...
                next_review_date = get_user_latest_payment_date.replace(
                    tzinfo=None
            ...
            return Response(
                ResponseData.success_for_get_reviews(
                    ...
                    can_user_submit_review,
                    str(next_review_date).split(" ")[0],
                ),
                status=status.HTTP_201_CREATED,
            )
        return Response(
...
def delete_review_by_id(request):
    ...
    try:
        ...
        if serializer.is_valid():
            ...
            ReviewModel.objects.values().filter(id=review_id).delete()
            return Response(
...
def delete_all_review(request):
    ...
    try:
        ...
        serializer = GetReviewSerializer(data=data)
        if serializer.is_valid():
            ReviewModel.objects.values().filter().delete()
            return Response(
                ResponseData.success_without_data("All review deleted successfully"),
                status=status.HTTP_201_CREATED,
            )
        return Response(
            ResponseData.error(serializer.errors),
...
def update_review(request):
    ...
    try:
        ...
        if serializer.is_valid():
            ...
            review_data = ReviewModel.objects.filter(id=review_id).first()
            if not review_data:
                return Response(
                    ResponseData.error("Review id is invalid."),
                    status=status.HTTP_200_OK,
            ...
            review_data.save()
            updated_data = list(ReviewModel.objects.values().filter(id=review_id))
            return Response(
                ResponseData.success(
