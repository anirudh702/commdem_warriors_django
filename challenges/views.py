...
from challenges.serializers import GetChallengesdataSerializer
...
@api_view(["POST"])
def get_workout_challenge_of_today_of_user(request):
    ...
    try:
        ...
        serializer = GetChallengesdataSerializer(data=data)
        if serializer.is_valid():
            user_id = serializer.data["user_id"]
            todays_date = str(datetime.now()).split(" ")[0]
            commitment_data = CommitmentModel.objects.values().filter(user_id=user_id,category_id=3).filter(Q(commitment_date__icontains=todays_date)).get()
