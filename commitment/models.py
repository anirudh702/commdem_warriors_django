...
class CommitmentNameModel(models.Model):
    ...
    successName = models.CharField(
        max_length=200, null=False, unique=True, db_index=True
    )
    failureName = models.CharField(
        max_length=200, null=False, unique=True, db_index=True
    )
    currentDayName = models.CharField(
        max_length=200, null=False, unique=True, db_index=True
    )
    created_at = models.DateTimeField(default=django.utils.timezone.now, blank=True)
    updated_at = models.DateTimeField(default=django.utils.timezone.now, blank=True)
...
class CauseOfCategorySuccessOrFailureModel(models.Model):
    ...
    id = models.AutoField(primary_key=True)
    category = models.ForeignKey(
        CommitmentCategoryModel, on_delete=models.CASCADE, null=False
    )
...
class CommitmentModel(models.Model):
    ...
    id = models.AutoField(primary_key=True)
    # user = models.ForeignKey(UserModel, on_delete=models.CASCADE,null=True,default=None)  # noqa: E501
    user_id = models.IntegerField(blank=True, default=0)
    total_commitments_done = models.IntegerField(blank=True, default=0)
    category = models.ForeignKey(
        CommitmentCategoryModel, on_delete=models.CASCADE, null=True
    )
    commitment_name = models.ForeignKey(
        CommitmentNameModel, on_delete=models.CASCADE, null=True
    )
...
class ReasonBehindCommitmentSuccessOrFailureForUser(models.Model):
    """Model for storing reasons behind success or failure of a particular commitment of a user"""  # noqa: E501

    id = models.AutoField(primary_key=True)
    # user = models.ForeignKey('user.UserModel', on_delete=models.CASCADE,null=False,default=None)  # noqa: E501
    user_id = models.IntegerField(blank=True, default=0)
    commitment = models.ForeignKey(
        CommitmentModel, on_delete=models.CASCADE, null=False
    )
    cause_of_category_success_or_failure = models.ForeignKey(
        CauseOfCategorySuccessOrFailureModel, on_delete=models.CASCADE, null=False
    )
    created_at = models.DateTimeField(default=django.utils.timezone.now, blank=True)
    updated_at = models.DateTimeField(default=django.utils.timezone.now, blank=True)
    objects = models.Manager()
...
class CommitmentGraphDataModel(models.Model):
    ...
    id = models.AutoField(primary_key=True)
    # user = models.ForeignKey('user.UserModel', on_delete=models.CASCADE,null=True,default=None)  # noqa: E501
    user_id = models.IntegerField(blank=True, default=0)
...
class ExerciseModel(models.Model):
    ...
    time_to_start = models.CharField(
        max_length=50, null=False, db_index=True, default=""
    )
    commitment_date = models.DateTimeField(default=next_day_datetime, blank=True)
